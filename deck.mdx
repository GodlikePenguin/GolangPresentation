...
### The `sync` package
- Go has a number of built in methods and structs to help you code concurrently: **WaitGroup**, **Mutex**
---
## WaitGroup
...
func sqrt(in <-chan float64, out chan<- answer, wg *sync.WaitGroup, id int) {
	//loop through the input numbers and place the resulting square root in the output channel
	//This loop will keep running until the 'in' channel is closed
	for num := range in {
